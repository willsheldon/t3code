...
- \`run_scheduled_task_now\` asks the scheduler to dispatch one existing task immediately without changing its definition or enabled state. Supply \`scheduledTaskId\` and a stable \`clientRequestId\`; reuse that key only for an exact retry. An accepted result identifies the target run but does not mean the agent turn has finished.
